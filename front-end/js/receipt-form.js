...
$(document).ready(function() {
    ...
    // Cập nhật giao diện theo loại phiếu
    ...
    } else if (receiptType === "outbound") {
        $("#formTitle").html('<i class="bx bx-export text-danger"></i> Tạo phiếu xuất');
        $(".btn-primary").addClass("btn-danger").removeClass("btn-primary");
    } else if (receiptType === "audit") {
        $("#formTitle").html('<i class="bx bx-check-shield text-warning"></i> Tạo phiếu kiểm kê');
        $("#partner").parent().find("label").text("Ghi chú kiểm kê");
        $("#partner").attr("placeholder", "Nhập mục đích kiểm kê...");
...
function addProductRow() {
    ...
    const isAudit = receiptType === "audit";

    let productHtml = `<select class="form-select product-select" data-index="${index}"><option value="">Chọn sản phẩm</option>`;
    productOptions.forEach(p => {
        productHtml += `<option value="${p.product_name}">${p.sku} - ${p.product_name}</option>`;
    });
    productHtml += "</select>";
    ...
    const rowHtml = `
        <div class="product-row" id="productRow_${index}">
            <div class="row">
                ...
                <div class="col-md-3">
                    <label class="form-label small">${isAudit ? 'Số lượng thực tế' : 'Số lượng'}</label>
                    <input type="number" class="form-control product-qty" data-index="${index}" value="0" min="0">
                </div>
                <div class="col-md-3" ${isAudit ? 'style="display:none"' : ''}>
                    <label class="form-label small">Đơn giá</label>
                    <input type="number" class="form-control product-price" data-index="${index}" value="0" min="0">
                </div>
...
function submitReceipt() {
    ...
    const data = {
        ...
        items: items.map((item) => ({
            ...
            price: receiptType == 'audit' ? 0 : item.price,
        })),
    };

    console.log("📦 Đang gửi dữ liệu:", data);

    let apiUrl = url + "/receipts/inbound";
    if (receiptType === "outbound") apiUrl = url + "/receipts/outbound";
    if (receiptType === "audit") apiUrl = url + "/receipts/audit";
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            setTimeout(() => { window.location.href = "receipt-table.html"; }, 1500);
        },
        error: function(xhr) {
            if (xhr.status === 401) {
                logout();
